    ...
    <section className="rounded-2xl border border-[#dbe5ef] dark:border-[#243443] bg-white dark:bg-[#101e2e] p-5 sm:p-6">
      ...
      <div className="mt-4 space-y-4">
        {guests.map((guest, index) => (
          <div
            key={`guest-${index}`}
            className="rounded-xl border border-[#dbe5ef] dark:border-[#243443] p-4"
          >
            <div className="mb-3 flex items-center gap-2 font-mulish text-sm font-bold text-[#101b25] dark:text-white">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <input
                ...
                className="rounded-md border border-[#cfd9e3] dark:border-[#3a4a5a] dark:bg-[#101e2e] dark:text-[#e8edf2] px-3 py-2 font-open-sans text-sm outline-none focus:border-primary"
              />
              <input
                type="email"
                value={guest.email}
            ...
            {errors[index] ? (
              <p className="mt-2 font-open-sans text-xs text-red-600 dark:text-red-400">
                {errors[index]}
              </p>
            ) : null}
          </div>
